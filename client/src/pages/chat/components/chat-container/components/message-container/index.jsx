  ...
  const renderChannelMessages = (message)=>{
    ...
    {message.messageType === "text" && (
      ...
      )}
      {
            ...
            <div className="flex items-center justify-start gap-3 ">
                <Avatar className=" h-8 w-8  rounded-full overflow-hidden ">
                    {message.sender.image && (
                      ...
                      <AvatarFallback
                        className={` uppercase h-8 w-8 md:w-48  text-lg flex items-center justify-center rounded-full ${getColor(
                            message.sender.color
                        )}`}
                      >
                        {message.sender.firstName
                          ? message.sender.firstName.trim().split("").shift()
                          : message.sender.email.trim().split("").shift()}
                      </AvatarFallback>
                  </Avatar>
        ...
        ):( <div className="text-sm text-white/60 mt-1 pr-1">
            {
              moment(message.timestamp).format("LT")
            }
            </div>)
